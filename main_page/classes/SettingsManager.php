...
class SettingsManager {
    ...
    public function __construct($studentId) {
        ...
        // Clean and validate student_id - remove any hashing if present
        $this->studentId = $this->cleanStudentId($studentId);
    }

    
     // Clean student ID - handle both regular IDs and hashed anonymous IDs
    ...
    public function getStudentInfo() {
        ...
        $stmt = $this->db->prepare(
            "SELECT student_id, first_name, middle_initial, last_name, email, phone_number 
    ...
    public function updateAccountInfo($firstName, $lastName, $middleInitial, $email, $phoneNumber) {
        if (!$this->studentExists()) {
        ...
        $stmt = $this->db->prepare(
            "UPDATE student 
             SET first_name = ?, last_name = ?, middle_initial = ?, email = ?, phone_number = ? 
             WHERE student_id = ?"
        );
        ...
        $stmt->bind_param("ssssss", $firstName, $lastName, $middleInitial, $email, $phoneNumber, $this->studentId);
        $result = $stmt->execute();
    ...
    public function updatePassword($newPassword) {
        // Use SHA-256 to match your existing password format
    ...
    public function getPreferences() {
        ...
        $stmt = $this->db->prepare(
            "SELECT language, theme FROM user_preferences WHERE student_id = ?"
        );
        
        if (!$stmt) {
            error_log("Failed to prepare preferences query: " . $this->db->error);
            return ['language' => 'en', 'theme' => 'system'];
        }
        ...
        if (!$prefs) {
            $this->updatePreferences('en', 'system');
            return ['language' => 'en', 'theme' => 'system'];
        }
