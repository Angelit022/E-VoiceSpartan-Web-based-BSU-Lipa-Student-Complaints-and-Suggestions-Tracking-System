...
profileToggle.addEventListener("click", () => {
  profileSub.style.display =
    profileSub.style.display === "block" ? "none" : "block";
  profileToggle.querySelector(".arrow").textContent =
    profileSub.style.display === "block" ? "▼" : "▶";
});

settingsToggle.addEventListener("click", () => {
  settingsSub.style.display =
    settingsSub.style.display === "block" ? "none" : "block";
  ...
  selectedCategories.forEach((cat, i) => {
    descBox.innerHTML += `
      <div style="margin-bottom:16px;">
        ...
        <textarea id="desc-${i}" placeholder="Describe your concern about ${cat}..."></textarea>
      </div>`;
  });
  ...
  selectedCategories.forEach((cat, i) => {
    uploadBox.innerHTML += `
      <div style="margin-bottom:30px;text-align:left;">
        ...
        <div class="file-preview" id="preview-${i}">No file chosen</div>
      </div>`;
  });
...
function goToStep4(descriptions) {
  ...
  selectedCategories.forEach((cat, i) => {
    const filesInput = document.getElementById(`file-${i}`);
    const fileNames =
      filesInput && filesInput.files.length > 0
        ? [...filesInput.files].map((f) => f.name).join(", ")
        : "No file attached";
    ...
    reviewWrap.innerHTML += `
      <div class="review-item">
        <h3>${cat}</h3>
        <p><strong>Description:</strong> ${descriptions[i] || "(none)"}</p>
        <p><strong>Attached:</strong> ${fileNames}</p>
      </div>`;
  });
