...
if (strpos($phone_input, '+63') === 0) {
    $phone_normalized = '0' . substr($phone_input, 3);
} elseif (strpos($phone_input, '63') === 0 && strpos($phone_input, '+') !== 0) {
    $phone_normalized = '0' . substr($phone_input, 2);
} else {
...
if ($is_admin) {
    ...
    if (!$stmt) {
        error_log("[SMS OTP] Database prepare failed: " . $conn->error);
...
if (!$stmt->execute()) {
    error_log("[SMS OTP] Failed to save OTP: " . $stmt->error);
...
if ($curlError) {
    error_log("[SMS OTP] CURL Error: " . $curlError);
    echo json_encode(['status' => false, 'message' => 'SMS service connection error. Please try again.']);
    exit;
}

if (!$response) {
    error_log("[SMS OTP] Empty response from SMS service");
...
if ($res === null) {
    error_log("[SMS OTP] Invalid JSON response from SMS service");
...
if ($isSuccess) {
    ...
    error_log("[SMS OTP] API returned error: " . json_encode($res));
